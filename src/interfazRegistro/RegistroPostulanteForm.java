...
public class RegistroPostulanteForm extends javax.swing.JFrame {
    ...
    // DAO y datos del postulante
    private DAO objDAO;
    private Postulante objPostulante;
    
    // Constructor sin parámetros (para compatibilidad)
    public RegistroPostulanteForm() {
        this.objPostulante = null;
        initialize();
    }
    
    // Constructor con postulante
    public RegistroPostulanteForm(Postulante postulante) {
    ...
    private void initialize() {
        ...
        // Si hay un postulante, prellenar el campo celular
        if (objPostulante != null) {
            // Aquí podrías prellenar datos si los tuvieras en el objeto Postulante
            // Por ejemplo: jTextField1.setText(objPostulante.getCelular());
        }
    }
    
    // Método para establecer el postulante desde fuera
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {
        ...
        // Verificar que tengamos un postulante registrado
        if (objPostulante == null) {
            JOptionPane.showMessageDialog(this,
                "Error: No se ha encontrado información del postulante.\n" +
                "Por favor, complete primero el registro de datos personales.",
                "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            String strCodigoCertificado = jTextField10.getText().trim();
            String strMunicipio = jTextField7.getText().trim();
            String strDepartamento = (String) jComboBox1.getSelectedItem();
            String strFacultad = (String) jComboBox2.getSelectedItem();
            String strCarrera = (String) jComboBox3.getSelectedItem();
            String strCelular = jTextField1.getText().trim();
            
            // Verificar si el código de certificado ya existe (si no está vacío)
            if (!strCodigoCertificado.isEmpty() && 
                objDAO.existeCodigoCertificadoPromo(strCodigoCertificado)) {
                JOptionPane.showMessageDialog(this,
                    "El código de certificado de bachiller ya está registrado en el sistema.",
                    "Código duplicado", JOptionPane.WARNING_MESSAGE);
                jTextField10.requestFocus();
                return;
            }
            
            // Crear objeto RegistroDocumentacion
            // Estados iniciales: documentación=false, pago=false
            RegistroDocumentacion objRegistroDoc = new RegistroDocumentacion(
                0, // ID se genera automáticamente
                objPostulante.getIntIdUsuario(), // ID del usuario
                strCodigoCertificado.isEmpty() ? null : strCodigoCertificado, // Código certificado (puede ser null)
                false, // Estado documentación (inicial: false)
                null, // Fecha registro admin (null hasta que lo apruebe)
                false, // Estado de pago (inicial: false)
                strMunicipio,
                strDepartamento,
                strFacultad,
                strCarrera,
            ...
            RegistroDocumentacion objRegistroCreado = objDAO.insertarRegistroDocumentacion(objRegistroDoc);
            
            if (objRegistroCreado != null) {
                String mensaje = String.format(
                    ...
                    objPostulante.getStrNombre(),
                    objPostulante.getStrApellido(),
                    objPostulante.getStrNumeroCI(),
                    jTextField2.getText(),
                    strDepartamento,
                    strMunicipio,
                    strFacultad,
                ...
                // Preguntar si desea registrar otro postulante
                int opcion = JOptionPane.showConfirmDialog(this,
                    "¿Desea registrar otro postulante?",
                    "Continuar", JOptionPane.YES_NO_OPTION);
                    
                if (opcion == JOptionPane.YES_OPTION) {
                    // Ir al inicio del registro
                    RegistroUsuarios registro = new RegistroUsuarios();
                    registro.setVisible(true);
                    registro.setLocationRelativeTo(null);
                    this.dispose();
                } else {
                    // Volver al menú principal o cerrar
                    try {
            ...
            logger.log(java.util.logging.Level.SEVERE, "Error al guardar registro de documentación", e);
            JOptionPane.showMessageDialog(this,
                "Error inesperado al guardar los datos:\n" + e.getMessage(),
                "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private boolean validateFields() {
        ...
        if (jTextField2.getText().trim().isEmpty()) {
            JOptionPane.showMessageDialog(this, 
                "Por favor, ingrese el nombre del colegio", 
        ...
        if (jComboBox3.getSelectedIndex() == 0 || 
            jComboBox3.getSelectedItem().toString().contains("Seleccione primero")) {
            JOptionPane.showMessageDialog(this, 
        ...
        // Validar celular
        String celular = jTextField1.getText().trim();
        if (celular.isEmpty()) {
            JOptionPane.showMessageDialog(this, 
        ...
        // Validar formato de celular (solo números, mínimo 7 dígitos)
        if (!celular.matches("\\d{7,}")) {
            JOptionPane.showMessageDialog(this,
                "El número de celular debe contener al menos 7 dígitos",
                "Formato inválido", JOptionPane.WARNING_MESSAGE);
            jTextField1.requestFocus();
            return false;
        }
        
        // Validar código de certificado si se ingresó (opcional)
        String codigo = jTextField10.getText().trim();
        if (!codigo.isEmpty() && !codigo.matches("\\d+")) {
            JOptionPane.showMessageDialog(this,
                "El código de certificado debe contener solo números",
                "Formato inválido", JOptionPane.WARNING_MESSAGE);
            jTextField10.requestFocus();
            return false;
        }
